...
from app.db.models import Account, Listing, ListingReport, UserProfile
from app.db.session import get_db

user_router = APIRouter(prefix="/listing-reports", tags=["listing-reports"])


class CreateListingReportPayload(BaseModel):
    listing_id: int
    reporter_id: int
    reason: str
...
def create_listing_report(
    payload: CreateListingReportPayload = Body(...),
    ...
    if listing.listing_type == "looking_for":
        raise HTTPException(
            ...
            detail={"error": "Use looking-for reports for this listing"},
        )
